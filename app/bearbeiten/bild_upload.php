...
if ((int)$cntStmt->fetchColumn() >= IMG_MAX_COUNT) {
    echo json_encode(['error' => 'Maximal ' . IMG_MAX_COUNT . ' Bilder erlaubt.']);
    exit;
}
...
if (!in_array($mime, $allowed)) {
    echo json_encode(['error' => 'Nur JPEG, PNG und WebP erlaubt']);
    exit;
}

[$width, $height] = getimagesize($file['tmp_name']);
if ($width < IMG_MIN_WIDTH) {
    echo json_encode(['error' => "Bild zu schmal – mindestens " . IMG_MIN_WIDTH . " px Breite erforderlich (dieses: {$width} px)"]);
    exit;
}
